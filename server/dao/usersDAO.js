...
class UsersDAO {
  static async injectDB(conn) {
    ...
    try {
      usersCollection = await conn.db(keys.dbName).collection("users")
      sessionsCollection = await conn.db(keys.dbName).collection("sessions")
    } catch (e) {
  ...
  static async logoutUser(email) {
    try {
      await sessionsCollection.deleteOne({ user_id: email })
      return { success: true }
    } catch (e) {
  ...
  static async getUserSession(email) {
    try {
      ...
      return sessionsCollection.findOne({ user_id: email })
    } catch (e) {
  ...
  static async makeAdmin(email) {
    try {
      const updateResponse = usersCollection.updateOne(
        { email },
        { $set: { isAdmin: true } },
      )
      return updateResponse
